...
export function useCategories(userId) {
  ...
  const addCategory = useCallback(async (label, color) => {
    ...
    if (userId) {
      const { data } = await supabase.from('categories').insert({ user_id: userId, value, label: label.trim(), color }).select().single();
      if (data) setCustomCategories((prev) => prev.map(c => c.value === value ? { ...c, id: data.id } : c));
    }
    return newCat;
  ...
  const deleteCategory = useCallback(async (value) => {
    ...
    if (userId && cat?.id) await supabase.from('categories').delete().eq('id', cat.id);
  }, [customCategories, userId]);
