...
export default function PrayerCard({
  ...
  const handleLogPrayed = () => {
    onLogPrayed();
    setJustLogged(true);
    setTimeout(() => setJustLogged(false), 2000);
  };

  const handleMarkAnswered = () => { setMenuOpen(false); setShowTestimonyPrompt(true); };
            ...
            <>
              ...
              <div className="prayer-card-menu">
                ...
                {!confirmDelete ? (
                  ...
                  <button className="menu-item menu-item-danger" onClick={() => handleAction(onDelete)}>
      ...
      {showTestimonyPrompt && (
        <div className="testimony-prompt">
          ...
          <div className="testimony-prompt-actions">
            <button className="btn btn-sm btn-secondary" onClick={() => setShowTestimonyPrompt(false)}>Cancel</button>
            <button className="btn btn-sm btn-primary" onClick={handleConfirmAnswered}>
      ...
      {!prayer.answered && (
        <div className="prayer-log-bar">
          ...
          <div className="prayer-log-actions">
            {prayerCount > 1 && (
              <button className="btn-log-undo" onClick={onUndoLog} title="Undo last">
                <Undo2 size={12} />
              </button>
            )}
          ...
          {isTimerRunning ? (
            <>
              <div className="prayer-timer-info">
                ...
                <span className="prayer-timer-elapsed">{formatDuration(timerElapsed)}</span>
                <span className="prayer-timer-label">in prayer</span>
              </div>
              <button className="btn-timer-stop" onClick={onStopTimer}>
                <Square size={11} /> Stop
              </button>
            </>
          ) : (
            <>
              ...
              <button className="btn-timer-start" onClick={onStartTimer}>
                <Timer size={13} /> Start
              </button>
            </>
          )}
                ...
                {notes.map((note) => {
                  ...
                  return (
                    <div key={note.id} className="prayer-note" style={{ borderLeftColor: nt.color }}>
                      ...
                      {!prayer.answered && (
                        <button className="prayer-note-delete" onClick={() => onDeleteNote(note.id)} aria-label="Delete note">
                          <X size={11} />
                        </button>
                      )}
                    </div>
